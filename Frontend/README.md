...
**Nurse**
- **Dashboard:** today's visits, visits waiting for check-out, referrals, low/out-of-stock alerts, weekly visit trend, top complaints. You can pick Today / This Week / This Month or a custom date range.
- **Visit Records:** searchable, filterable (gender, year level, program, department, date range), sortable, and paginated. Export to CSV, or print / save as PDF.
- **Add Patient Record:** search the roster to reuse an existing patient, or register a new one. Record vitals, complaints and treatment, tag complaints for reports, and dispense medicines and supplies (they're deducted from stock automatically).
- **Today's Visits panel:** sign a patient out with a disposition (Sent Home, Back to Class/Work, Hospital Admission, and so on).
- **Edit / soft-delete** visits and patients. **Patient Profile** shows the full visit history.
...
- **Stock & Supplies:** live inventory with High/Low/Out status. Add or restock items, request a restock from Admin, and confirm deliveries.
- **Reports & Insights:** symptom/treatment patterns and the most common complaints.

**Admin**
- **Dashboard:** approve or deny nurse restock requests, see account and stock status.
...
- The **backend** running on `http://localhost:8000` (see [Connecting to the backend](#connecting-to-the-backend))
