...
### Before you start: know your two folders

Every command below runs in one of these. Find yours once and keep them handy.

| Referred to as | What it means | Example on my machine |
|---|---|---|
| **your repo folder** | where you cloned the repo | `C:\Users\com sci\Logbook-git` |
| **your backend folder** | `BackendAndDatabase\backend` inside it | `C:\Users\com sci\Logbook-git\BackendAndDatabase\backend` |
...
Keep the quotes — the path has a space in it.

### Use PowerShell, not Command Prompt

Every command in this guide is PowerShell. Command Prompt (cmd) does not
understand `$env:Path`, `Set-Content`, or `.\.venv\Scripts\Activate.ps1`, and
you will get "The filename, directory name, or volume label syntax is
incorrect." Open PowerShell from the Start menu, or in File Explorer press
Shift + right-click inside the folder and choose "Open PowerShell window here".

---

## 1. Load the database — FIRST INSTALL ONLY

> **Skip this entire step if the database already exists.**
> `schema_v2.sql` begins with `DROP DATABASE IF EXISTS isu_infirmary`.
> Running it a second time deletes every table, every patient record and every
> user account, with no undo. Run the verification query at the end of this
> step FIRST — if it returns 22, the database is already loaded and you go
> straight to Step 2.
...
redirect operator, so hand that one command to cmd:

```powershell
cd "your repo folder\BackendAndDatabase"
```
```powershell
...
If you get `'mysql' is not recognized`, the PATH line above did not run, or
XAMPP is installed somewhere other than `C:\xampp`.
...
Then, one command at a time (pasting several at once gives you a `>>`
continuation prompt instead of running them):
...
2. In PowerShell: `cd "your backend folder"` then `.\.venv\Scripts\Activate.ps1`
